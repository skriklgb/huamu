...
function get_maincat_all($catid,$moduleid) {

	global $db;

	$catid = intval($catid);

	$condition ="parentid=$catid and moduleid=$moduleid";
...
function get_fenlei($CAT, $str = ' &nbsp; ', $target = '') {
...
function cuntsp($username) {
    global $db;
    $r=$db->get_one("SELECT count(*) as sum FROM {$db->pre}mall WHERE username='".$username."'");
	return $r['sum'];
